...
我们将以官方筛选插件为例进行介绍一个 Univer 插件的结构，读者们可以在此基础上深入源码以及 [API Reference](/typedoc/@univerjs/README) 了解如何撰写一个复杂的插件。
...
<Callout type="info" emoji="💡">
  了解 Univer 架构的 [分层模型](/guides/sheets/architecture/univer#以分层架构划分模块)。
</Callout>
...
<Callout type="info" emoji="💡">
  了解 [快照](/guides/sheets/getting-started/concepts#快照) 的概念。
</Callout>
...
<Callout type="info" emoji="💡">
  阅读 [命令系统](/guides/sheets/architecture/univer#命令系统) 了解什么是命令以及命令的三种类型。
</Callout>

<Callout type="info" emoji="💡">
  阅读 [扩展命令](guides/sheet/advanced/custom-command) 了解如何自定义命令。
</Callout>
...
<Callout type="info" emoji="💡">
  一些插件需要拓展复制粘贴或下拉填充，[扩展命令](guides/sheet/advanced/custom-command) 章节也介绍了如何对这两个操作进行拓展。
</Callout>
...
<Callout type="info" emoji="💡">
  请阅读 [渲染引擎架构设计](/guides/sheets/architecture/rendering) 了解如何撰写渲染层代码。
</Callout>
...
<Callout type="info" emoji="💡">
  请阅读 [自定义 UI](guides/sheet/advanced/custom-ui) 了解如何自定义包括 mutation 在内的命令。
</Callout>
...
<Callout type="info" emoji="💡">
  关于 Univer 的生命周期机制，请查看文档 [插件生命周期](/guides/sheets/architecture/univer#插件生命周期)。
</Callout>

<Callout type="info" emoji="💡">
  Univer 使用依赖注入模式来管理模块之间的依赖关系以及实例化模块。关于依赖注入的更多介绍，请参考我们的 [文档](/guides/sheets/architecture/univer#依赖注入)。
</Callout>
