...
<Steps>
  ...
  <Callout emoji="⚠️">
    如果在安装或使用过程中遇到问题，请查看[常见问题](/guides/sheets/server/docker#troubleshooting)。
  </Callout>

  ### 安装并运行客户端 <a id="install-and-run-the-frontend" />

  <Tabs items={['pnpm', 'npm', '从单 HTML 文件开始']}>
    <Tabs.Tab label="pnpm">
      执行以下命令需要设备安装有 `nodejs >= 18`, `pnpm >= 7` 和 `git`。
...
- **🔰 安装和引入**：如果你想学习如何从头开始手动创建一个 Univer 项目，请查看 [安装和引入](/guides/sheets/getting-started/installation) ，它将一步步引导你完成整个过程。
