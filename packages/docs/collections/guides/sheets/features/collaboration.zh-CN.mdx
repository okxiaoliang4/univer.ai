...
1. 通过 [创建文档](/guides/sheets/server/api#create-empty-document) 接口可以创建一篇空白文档。
2. 通过 [导入插件](/guides/sheets/features/import-export) 提供的 Facade API [`importXLSXToUnitId`](/guides/sheets/features/core/sheet-api#import-xlsx-and-get-unitid) 方法，将 XlSX 文件导入到 Univer 服务。
