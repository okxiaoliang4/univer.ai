...
2. 定义国际化
    ...
    定义你所需要的国际化内容，详细的字段说明请参考 [如何在 Formula Engine 中添加公式](/guides/sheet/advanced/custom-formula/#如何在-formula-engine-中添加公式) 的部分。同样的，多个公式就用公式名称作为 `key` 值区分。
...
详细 API 参考 [FunctionType](/typedoc/@univerjs/engine-formula/enumerations/FunctionType)
