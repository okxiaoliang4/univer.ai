    ...
    >
      ...
      >
        <tr className="grid grid-flow-col items-center gap-9">
          <th className="w-[300px] pb-4 pt-10 text-left text-lg font-semibold">What's included in the plans</th>
          <th
            className="w-[312px] pb-4 pt-10 text-center text-xl font-bold"
          >
            Developer
          </th>
          <th
            className="w-[312px] bg-[rgba(87,80,183,0.2)] pb-4 pt-10 text-center text-xl font-bold"
          >
            Business
          </th>
        </tr>
        ...
        {plans.map(plan => (
          <Fragment key={plan.title}>
            <tr className="grid h-14 grid-flow-col items-center gap-9">
              <td className="flex h-full w-[300px] items-center text-lg font-bold">{plan.title}</td>
              <td className="flex h-full w-[312px] items-center px-6">
                <hr className="w-full border-dashed border-gray-300" />
              </td>
              <td className="flex h-full w-[312px] items-center bg-[rgba(87,80,183,0.2)] px-6">
                <hr className="w-full border-dashed border-gray-300" />
              </td>
            </tr>
            ...
            {plan.features.map(feature => (
              <Fragment key={feature.title}>
                <tr
                  ...
                  `}
                >
                  <td className="flex h-full w-[300px] items-center">
                    {'children' in feature
                      ? (
                          <a
                  ...
                  <td className="flex h-full w-[312px] items-center justify-center px-6">
                  ...
                  <td className="flex h-full w-[312px] items-center justify-center bg-[rgba(87,80,183,0.2)] px-6">
                    {!('children' in feature && feature.children) && (
                      <CheckMarkSingle />
                    )}
                  </td>
                ...
                {'children' in feature && feature.children?.map(child => (
                  <Fragment key={child.title}>
                    ...
                    >
                      <td className="flex h-full w-[300px] items-center pl-4 text-white/60">{child.title}</td>
                      <td className="flex h-full w-[312px] items-center justify-center px-6">
